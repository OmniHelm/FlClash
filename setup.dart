...
Future<void> _addPortableFlagToZip(File zipFile) async {
  ...
  final flagNames = archive.files
      .where(
        (file) =>
            file.isFile &&
            p.posix.basename(file.name).toLowerCase() ==
                '$_appName.exe'.toLowerCase(),
      )
      .map((file) {
        final parent = p.posix.dirname(file.name);
        return parent == '.'
            ? _portableFlagFileName
            : p.posix.join(parent, _portableFlagFileName);
      })
      .toSet();
  if (flagNames.isEmpty) {
    flagNames.add(_portableFlagFileName);
  }
