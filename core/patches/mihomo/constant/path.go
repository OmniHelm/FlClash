...
var Path = func() *path {
	...
	return &path{homeDir: homeDir, geoDir: homeDir, configFile: "config.yaml", allowUnsafePath: allowUnsafePath, safePaths: safePaths}
}()
	...
	for _, fi := range files {
		if fi.IsDir() {
			...
			if strings.EqualFold(fi.Name(), "Country.mmdb") ||
				...
				strings.EqualFold(fi.Name(), "GEOIP.metadb") {
				GeoipName = fi.Name()
				return P.Join(geoDir, fi.Name())
			}
		}
	}
	return P.Join(geoDir, "GEOIP.metadb")
}
...
func (p *path) ASN() string {
	...
	return P.Join(geoDir, ASNName)
}

func (p *path) OldCache() string {
	return P.Join(p.homeDir, "cache", ".cache")
}

func (p *path) Cache() string {
	return P.Join(p.homeDir, "cache", "cache.db")
}
...
func (p *path) GeoIP() string {
	...
	return P.Join(geoDir, "GEOIP.dat")
}
...
func (p *path) GeoSite() string {
	...
	return P.Join(geoDir, "GEOSITE.dat")
}

func (p *path) GetAssetLocation(file string) string {
	return P.Join(p.GeoDir(), file)
}
